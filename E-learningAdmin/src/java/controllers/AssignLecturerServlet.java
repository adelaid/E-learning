 ...
 * @author Ada
 */
@WebServlet(name = "AssignLecturerServlet", urlPatterns = {"/AssignLecturerServlet"})
public class AssignLecturerServlet extends HttpServlet {
      ...
      String lecturer=request.getParameter("lecturer");
        if(courses.length==0){
            ...
            for(int i=0;i<courses.length;i++){
              assignCoursetoLecturer(Long.parseLong(courses[i]), Long.parseLong(lecturer));
            }
    ...
    private void assignCoursetoLecturer(java.lang.Long courseId, java.lang.Long lecturerId) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        ws.CourseWS port = service.getCourseWSPort();
        port.assignCoursetoLecturer(courseId, lecturerId);
    }
